  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        {/* Group List Panel */}
        <div className="w-full md:w-72 bg-gray-900 rounded-lg p-4 shadow flex flex-col">
          <div className="mb-4">
            ...
            {createStatus && <div className="text-green-400 text-xs mt-1">{createStatus}</div>}
          </div>
          <div className="font-semibold text-gray-300 mb-2 pl-1">Your Groups</div>
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar max-h-[40vh]">
        ...
        {/* Group Details Panel */}
        <div className="flex-1 bg-gray-900 rounded-lg p-6 shadow min-h-[60vh] flex flex-col">
          {!selectedGroup ? (
            ...
            <>
              <div className="flex justify-between items-center mb-4">
                {editMode ? (
                  <form onSubmit={handleRename} className="flex flex-wrap gap-2">
                    ...
                    />
                    <button type="submit" className="bg-green-600 px-3 py-1 rounded text-white">
                      Save
                    </button>
                    <button
                      type="button"
              ...
              <div className="mb-4 text-sm">
                ...
                <span className="font-bold text-white">
                  {selectedGroup.owner.username || selectedGroup.owner.email}
                </span>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="max-h-40 overflow-y-auto pr-1 custom-scrollbar bg-gray-800/40 rounded p-2">
                  <ul className="space-y-2">
                    ...
                    {selectedGroup.members.length === 0 && (
                      <li className="text-gray-500 text-xs">No members.</li>
                    )}
                  </ul>
                </div>
              </div>
              ...
              {(selectedGroup.owner._id === user._id || user.role === "admin") && (
                <div className="mb-6">
                  ...
                  {userResults.length > 0 && (
                    <div className="bg-gray-800 rounded p-2 mb-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                      {userResults.map(u => (
                      ...
                      <button
                        type="button"
                        className={`mt-2 w-full font-semibold py-1 rounded ${
                          addUserIds.length === 0
              ...
              {/* Files in group (scrollable) */}
              <div className="mb-4">
                <div className="font-semibold text-gray-300 mb-1">Files in this group:</div>
                {files.length === 0 ? (
                  <div className="text-gray-500 text-sm">No files in this group.</div>
                ) : (
                  <div className="max-h-40 overflow-y-auto pr-1 custom-scrollbar bg-gray-800/40 rounded p-2">
                    <ul className="list-disc pl-5 text-gray-200 space-y-1 text-sm">
                      {files.map(f => (
                        <li key={f._id} className="truncate">{f.filename}</li>
                      ))}
                    </ul>
                  </div>
                )}
              ...
              {status && <div className="text-gray-300 mt-2 text-sm">{status}</div>}
            </>
          )}
        </div>
      </div>
